...
from template.agent.tools.read_pdf import retrieve
...
class Agent:
    ...
    def __init__(self, api_key: str, model: str = "gpt-4o-mini", prompt: str = SYSTEM_PROMPT, temperature: float = 0.2):
        ...
        # Store current user context for tools
        ...
        self.tools = [
            ...
            StructuredTool.from_function(
                func=lambda query: retrieve(query, user_id=self.current_user_id, session_id=self.current_session_id),
                name="retrieve",
                description="Truy xuất thông tin từ tài liệu PDF đã được tải lên của người dùng hiện tại. Input là chuỗi truy vấn tìm kiếm. Ví dụ: 'thông tin về công nghệ AI'. Output là các đoạn văn bản liên quan từ tài liệu.",
            )
        ]
    ...
    def chat(self, request: ChatRequest) -> ChatResponse:
        ...
        try:
            # Update current user context for tools
            self.current_user_id = request.user_id
            ...
            return ChatResponse(
                session_id=request.session_id if hasattr(request, 'session_id') else 0,
